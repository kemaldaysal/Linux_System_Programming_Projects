...
int main()
{
    ...
    printf("P3(%d): Semaphore opened successfully.\n", getpid());

    sem_getvalue(sem, &sval); //
    printf("P3(%d): Obtained semaphore value before sem_wait: %d\n", getpid(), sval);
    printf("P3(%d): I will try to gain access to semaphore if available, if not i'll wait...\n", getpid());
    // -------------- Critical section - start
    ret = sem_wait(sem); // wait state
    // after executing, sval = 1 - 1 = 0
    printf("P3(%d): Access gained\n", getpid());
    sem_getvalue(sem, &sval); // 0
    printf("P3(%d): Obtained semaphore value after sem_wait: %d\n", getpid(), sval);

    printf("P3(%d): Executing critical section\n", getpid());
    printf("P3(%d): Starting to sleep for %d seconds.\n", getpid(), SLEEP_TIME_S);
    sleep(SLEEP_TIME_S);
    printf("P3(%d): ret: %d\n", getpid(), ret);
    sem_post(sem); // 0 + 1 = 1
    // -------------- Critical section - end

    printf("P3(%d): Critical section finished executing\n", getpid());
    sem_getvalue(sem, &sval); // 1
    printf("P3(%d): Obtained semaphore value after sem_post: %d\n", getpid(), sval);
    printf("P3(%d): Notifying that i'm done by posting sem_done_cnt synchroniser.\n", getpid());
    sem_post(sem_done_cnt);

    printf("P3(%d): Closing semaphore on my side...\n", getpid());    
