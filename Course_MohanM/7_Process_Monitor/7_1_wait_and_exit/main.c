...
1. status value if child process has normal exit/termination: (we programmed it to return 3 by exit(3) statement on child.)
    15......8       |      7......0
...
int main()
{
    ...
    default:
        printf("--P: (PID: %d): Started executing, now i'll wait() for my childs (PID Child: %d) to finish executing\n", getpid(), cpid);
        ...
        if (status != 0 && status < 256) // kill signal generates a status signal between 2^0 (??) and 2^7 
        {
