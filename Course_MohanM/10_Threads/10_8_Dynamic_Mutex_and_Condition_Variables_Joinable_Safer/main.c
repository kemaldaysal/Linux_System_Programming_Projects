 ...
 *
 * Making threads joinable, not detachable is safer since we guarantee that main still has the mutex to destroy, and main does the cleanup  
 *
 */
...
int main()
{
    ...
    // CHECK_ERR(pthread_detach(t1), "Error detaching thread"); // not safe if multiples threads are used in dynamic mutex and conds
    ...
    // after signaling and unlocking:
    CHECK_ERR(pthread_join(t1, NULL), "Join failed");

    printf("-MT : t1 finished it's job, starting destroyers and freeing data struct.\n");

    // cleanup in main (safe now)
    CHECK_ERR(pthread_mutex_destroy(&data->mutex), "Mutex destroy failed");
    CHECK_ERR(pthread_cond_destroy(&data->cond_from_main), "Cond destroy failed (main)");
    CHECK_ERR(pthread_cond_destroy(&data->cond_from_t1), "Cond destroy failed (t1)");
    free(data);
